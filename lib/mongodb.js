...
const DATE_GROUPS = ['dayOfYear', 'dayOfMonth', 'dayOfWeek', 'year', 'month', 'week', 'hour', 'minute', 'second',
  'millisecond'];

class MongoDB extends JugglerAccessor {
    ...
    if (!_.isEmpty(query.fields)) {
      _.forEach(query.fields, field => {
        ...
        if (aggregation) { // measures
          ...
          } else if (_.includes(DATE_GROUPS, aggregation)) {
