...
module.exports = class MongoDB extends JugglerAccessor {
  ...
  buildExpression(exp, options) {
    const namemapper = (options && options.namemapper) || (name => name);
    function transform(node) {
      if (node.fn) {
        const fn = typeof node.fn === 'string' ? node.fn : node.fn.name;
        let args = null;
        if (node.args) {
          if (node.args.length === 1) {
            args = transform(node.args[0]);
          } else {
            args = node.args.map(arg => transform(arg));
          }
        }
        return {['$' +fn]: args}
    ...
    return transform(math.parse(exp));
  }
    ...
    if (!_.isEmpty(query.fields)) {
      _.forEach(query.fields, field => {
        ...
        if (field.fn && _.includes(['toUpper', 'toLower'], field.fn)) {
          ...
          project[name] = columnName ? ('$' + columnName) : this.buildExpression(expression, options);
        }
      });
    }
