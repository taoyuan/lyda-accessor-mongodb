      ...
      return Promise.fromCallback(callback => collection.aggregate(aggregation, callback)).then(docs => {
        const result = _.map(docs, doc => {
          ...
          _.forEach(item, (value, key) => {
            // query.fields should contains all field information from layer's field
            const field = _.find(query.fields, field => {
              return (field.name === key)
                || (_.find(['sum', 'avg', 'min', 'max', 'count'], aggregation => field.name + '_' + aggregation === key));
            });

            if (!field) return;

            // resolve local map values
            if (field.values) {
              const pair = _.find(field.values, pair => pair.value === value);
              if (pair) {
                item[key] = value = pair.value;
              }
            }

            // resolve date with format
            if (field.type === 'date') {
              item[key] = moment(value).format(field.format);
            }

            if (field.type === 'number') {
              item[key + '_original'] = value;
              item[key] = numeral(value).format(field.format);
            }
          });

          // added resource id|name to keys
          return _.transform(item, (result, value, key) => {
            result[query.id + '_' + key] = value;
            return result;
          }, {});
        });
